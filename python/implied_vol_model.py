...
def _newton_implied_vol(val_fn, option_type, x, fs, t, b, r, cp, precision=.00001, max_steps=100):
    ...
    # print(option_type, x, fs, t, b, r, cp, precision, max_steps)
    ...
    # print(value, delta, gamma, theta, vega, rho)
    # print(min_diff)

    # print("-----")
    # print("Debug info for: _Newton_ImpliedVol()")
    # print("    Vinitial={0}".format(v))
    ...
    while precision <= abs(cp - value) <= min_diff and countr < max_steps:
        ...
        countr += 1
        # print("     IVOL STEP {0}. v={1}".format(countr, v))

    # print(abs(cp - value), precision, v)

    # check if function converged and return a value
    if abs(cp - value) < precision:
        # the search function converged
        return v, round(delta, 2), round(gamma, 4), round(theta, 2), round(vega, 2), round(rho, 4)
    else:
        # if the search function didn't converge, try a bisection search
        return _bisection_implied_vol(val_fn, option_type, fs, x, t, r, b, cp, precision, max_steps), round(delta, 2), round(gamma, 4), round(theta, 2), round(vega, 2), round(rho, 4)
...
def ForwardPrice(Spot, Time, Interest, Yield):
    import math
    return round(Spot * math.exp((Interest - Yield) * Time), 2)


# forward price

def syntheticFuture(ce, pe, strike, Time, Interest):
    import math
    ert = math.exp(-Interest * Time)
    return round((ce - pe) / ert + strike, 2)


#skew

def skew_25_delta(param):
    return round( (param['put_iv'] - param['call_iv']) / param['atm_iv'] , 4 ) if param['atm_iv'] else 0
    # return round( (param['put_iv'] - param['call_iv']) / param['atm_iv'] , 4 )