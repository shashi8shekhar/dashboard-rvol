...
class OptionChain():
    ...
    """
    def __init__(self, api_key, api_secret, request_token, symbol, expiry, config):
        ...
        self.expiry = expiry
        self.config = config
        self.instrumentClass = InstrumentMaster(self.api_key, self.config)
    ...
    def create_option_chain(self):
        ...
        """
        self.socketClient = WebsocketClient(self.api_key, self.api_secret, self.request_token, self.symbol, self.expiry, self.config)
